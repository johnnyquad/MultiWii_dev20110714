...
//*********************************************************************************
  ...
  batRaw = analogRead(V_BATPIN); //my own version JDH ******************************
  ...
  if (currentTime > (rcTime + 20000) ) { // 50Hz
    ...
    if (rcData[THROTTLE] < MINCHECK) {
      ...
      if (rcData[YAW] < MINCHECK && rcData[PITCH] < MINCHECK && armed == 0) {
        if (rcDelayCommand == rcDelayValue) calibratingG=400;
      } else if (rcData[YAW] > MAXCHECK && rcData[PITCH] > MAXCHECK && armed == 0) {
        if (rcDelayCommand == rcDelayValue) {
        ...
        if (rcDelayCommand == rcDelayValue) armed = 0; // rcDelayCommand = 20 => 20x20ms = 0.4s = time to wait for a specific RC command to be acknowledged
      } else if ( (rcData[YAW] > MAXCHECK || rcData[ROLL] > MAXCHECK) && rcData[PITCH] < MAXCHECK && armed == 0 && calibratingG == 0 && calibratedACC == 1) {
        if (rcDelayCommand == rcDelayValue) armed = 1;
     #ifdef LCD_TELEMETRY_AUTO
      ...
      if (rcData[YAW] < MINCHECK && rcData[PITCH] < MINCHECK) {
        if (rcDelayCommand == rcDelayValue) calibratingA=400;
        rcDelayCommand++;
